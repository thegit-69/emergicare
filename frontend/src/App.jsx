    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                {['Admin', 'Doctor'].includes(user.role) && <button onClick={() => setIsAssessing(!isAssessing)} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">{isAssessing ? 'Cancel' : '+ New Assessment'}</button>}
            </div>
            {isAssessing && (
                        ...
                        <tbody>
                            {triageEntries.map(t => (
                                <tr key={t.id} className="border-b hover:bg-gray-50">
                                    <td className="p-3 text-sm text-gray-500">{new Date(t.assessedAt).toLocaleString()}</td>
...
const EmergencyQueueView = ({ emergencyQueue, setEmergencyQueue, triageEntries, user, fetchQueueData }) => {
                            ...
                            {emergencyQueue.map(q => (
                                <div key={q.id} className={`p-4 border-l-4 rounded-r-lg shadow-sm bg-white ${q.priorityScore === 1 ? 'border-red-500' : q.priorityScore === 2 ? 'border-orange-500' : 'border-yellow-500'} flex justify-between items-center`}>
                                    <div>
                                        ...
                                        <div className="text-sm text-gray-500 mt-1">Wait Time: {Math.floor((Date.now() - new Date(q.enqueuedAt).getTime()) / 60000)} mins</div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        {['Admin', 'Doctor'].includes(user.role) && (
                                            <>
                                                ...
                                                {q.queueStatus === 'In Treatment' && <button onClick={() => handleDischarge(q.id)} className="text-xs bg-green-100 text-green-700 px-3 py-1 rounded hover:bg-green-200">Discharge</button>}
                                            </>
                                        )}
                                    </div>
                                </div>
                            ))}
                            {emergencyQueue.length === 0 && <p className="text-gray-500 italic">The emergency queue is currently empty.</p>}
                        </div>
                    </Card>
                </div>
                        ...
                        <div className="space-y-3">
                            {waitingTriages.map(t => (
                                <div key={t.id} className="p-3 border rounded-lg flex justify-between items-center bg-gray-50">
                                    ...
                                    <button onClick={() => handleEnqueue(t.id, t.patientId)} className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded font-semibold hover:bg-red-200">Enqueue</button>
                                </div>
                            ))}
                ...
                {resources.map(r => (
                    <div key={r.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                        <div className="flex justify-between items-start mb-2">
                            <h3 className="font-bold text-gray-800">{r.resourceName}</h3>
                        ...
                        <p className="text-sm text-gray-500 mb-4">{r.resourceType} - {r.location}</p>
                        {['Admin', 'Doctor'].includes(user.role) && (
                            <button 
...
const AppLayout = ({ user, onLogout }) => {
    ...
    const fetchQueueData = async () => {
        const [trData, qData] = await Promise.all([api.getTriageEntries(), api.getEmergencyQueue()]);
        setTriageEntries(trData); setEmergencyQueue(qData);
    };
    ...
    const NavLink = ({ viewName, children, requiredRoles, icon }) => { 
        if (requiredRoles && !requiredRoles.includes(user.role)) return null; 
        return (
            <button onClick={() => { setActiveView(viewName); setSelectedPatient(null); setIsSidebarOpen(false); }} 
                    className={`w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center gap-3 ${activeView === viewName ? 'bg-red-600 text-white shadow-md' : 'text-gray-700 hover:bg-red-50 hover:text-red-700'}`}>
                {icon}
                {children}
            </button>
        ); 
    ...
    const renderView = () => {
        ...
        switch (activeView) {
            case 'triage': return <TriageView triageEntries={triageEntries} setTriageEntries={setTriageEntries} patients={patients} user={user} />;
            case 'queue': return <EmergencyQueueView emergencyQueue={emergencyQueue} setEmergencyQueue={setEmergencyQueue} triageEntries={triageEntries} user={user} fetchQueueData={fetchQueueData} />;
            case 'resources': return <ResourceManagementView resources={resources} setResources={setResources} user={user} fetchResources={fetchResourcesData} />;
