...
const Card = ({ title, children, className, actions }) => (
    <div className={`bg-white rounded-xl shadow-md ${className || ''}`}>
        {title && <div className="p-4 md:p-6 border-b border-gray-200 flex flex-col sm:flex-row justify-between sm:items-center gap-4"><h2 className="text-lg md:text-xl font-bold text-gray-800">{title}</h2><div className="flex flex-wrap gap-2">{actions}</div></div>}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                    <Card title="Active Queue (Priority Ordered)">
                        <div className="space-y-4">
                            {emergencyQueue.filter(q => q.queueStatus !== 'Discharged').map(q => (
                                <div key={q.id} className={`p-4 border-l-4 rounded-r-lg shadow-sm bg-white ${q.priorityScore === 1 ? 'border-red-500' : q.priorityScore === 2 ? 'border-orange-500' : 'border-yellow-500'} flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4`}>
                                    <div>
                    ...
                    <Card title="Discharged Patients History">
                        <div className="space-y-4">
                            {emergencyQueue.filter(q => q.queueStatus === 'Discharged').map(q => (
                                <div key={q.id} className="p-4 border rounded-lg bg-gray-50 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                                    <div>
                    ...
                    <Card title="Pending Transfers">
                        ...
                        <div className="space-y-3">
                            {waitingTriages.map(t => (
                                <div key={t.id} className="p-3 border rounded-lg flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 bg-gray-50">
                                    <div>
...
const AppLayout = ({ user, onLogout }) => {
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen bg-gray-50 overflow-hidden">
            {/* Mobile Header */}
            <div className="bg-red-600 text-white flex justify-between items-center md:hidden p-4 shadow-md z-30 shrink-0">
            ...
            <aside className={`bg-white shadow-xl w-64 h-full space-y-6 py-7 flex flex-col absolute md:relative inset-y-0 left-0 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 transition duration-200 ease-in-out z-40 shrink-0`}>
                <div className="px-6 text-center border-b pb-6 shrink-0">
                ...
                <div className="px-6 border-b pb-4 shrink-0">
                ...
                <nav className="flex-grow px-4 space-y-1 overflow-y-auto">
                    ...
                    <a href="https://eraktkosh.mohfw.gov.in/eraktkoshPortal/#/publicPages/bloodAvailabilitySearch" target="_blank" rel="noopener noreferrer"
                        className="w-full text-left px-4 py-3 rounded-lg text-sm font-medium text-gray-700 hover:bg-red-50 hover:text-red-700 transition-colors duration-200 flex items-center gap-3">
